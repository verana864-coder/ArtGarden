...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('cartItems') || document.getElementById('cartContainer')) {
        // Cart page will be handled by loadUserData after user data loads
    }
...
async function addToCart(artwork) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const error = await response.json();
            showNotification('Failed to add to cart: ' + error.error, 'error');
        }
    } catch (error) {
...
function updateCartUI() {
    ...
    if (cartBody) {
        if (cart.length === 0) {
            cartBody.innerHTML = '<p style="text-align: center; color: var(--text-muted); padding: 40px 0;">Your cart is empty</p>';
        } else {
    ...
    children.forEach(child => {
        ...
        card.innerHTML = `
            <div class="review-header">
                <img src="${child.image}" alt="${child.name}" class="review-avatar">
                <div class="review-info">
                    <h4>${child.name}</h4>
                    <div class="review-date">${child.age} years old • ${child.location}</div>
                </div>
            </div>
            ...
            <p class="review-text">${child.story.substring(0, 150)}...</p>
        `;
...
function displayChildProfile(child) {
    ...
    document.getElementById('childLocation').textContent = `Location: ${child.location}`;
    document.getElementById('childTalent').textContent = `Talent: ${child.talent}`;
    document.getElementById('childDream').textContent = `Dream: ${child.dream}`;
    document.getElementById('childImage').src = child.image;
...
// Mobile Menu
            <div class="invoice-details">
                <div class="invoice-row">
                    ...
                    <span class="invoice-value">${invoice.id}</span>
                </div>
                <div class="invoice-row">
                    <span class="invoice-label">Supporter Name:</span>
                    <span class="invoice-value">${invoice.supporterName}</span>
                </div>
                <div class="invoice-row">
                    <span class="invoice-label">Artwork:</span>
                    <span class="invoice-value">${invoice.artworkTitle}</span>
                </div>
                <div class="invoice-row">
                    <span class="invoice-label">Support Amount:</span>
                    <span class="invoice-value">$${invoice.amount}</span>
                </div>
                <div class="invoice-row">
                    <span class="invoice-label">Total Supporters:</span>
                    <span class="invoice-value">${invoice.supporterCount}/20</span>
                </div>
            ...
            <div class="invoice-actions">
                <button class="invoice-btn primary" onclick="closeModal()">Continue</button>
                <button class="invoice-btn secondary" onclick="printInvoice()">Print Invoice</button>
            </div>
        </div>
    `;
    
    showModal('invoice', modalContent);
}
                ...
                showModal('thankYou', `
                    <div class="modal-content">
                        ...
                        <div class="invoice-header">
                            ...
                            <p>Please help my next friend who needs your support.</p>
                        </div>
                        <div class="invoice-actions">
                            <button class="invoice-btn primary" onclick="closeModal()">Continue Supporting</button>
                        </div>
                    </div>
                `);
            }, 1000);
...
function showModal(type, customContent = null) {
        ...
        content = `
            <div class="modal-content">
                ...
                <div class="invoice-header">
                    ...
                    <p>Please login to support children's artwork</p>
                </div>
                <div class="invoice-actions">
                    <button class="invoice-btn primary" onclick="openAuthModal()">Login</button>
                    <button class="invoice-btn secondary" onclick="closeModal()">Cancel</button>
                </div>
            </div>
        `;
    }
    ...
    if (modalContainer) {
        modalContainer.innerHTML = `<div class="modal-overlay active" id="modal">${content}</div>`;
    }
}
...
function setupLoginForm() {
    ...
    console.log('Setting up login forms...', signinForm, signupForm);
